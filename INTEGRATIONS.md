...
**cloud-setups** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `cloud-setups` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
cloud-setups ... --format json | cognis-connect emit --to sigma   # Sigma rules
cloud-setups ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
cloud-setups ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
cloud-setups ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `cloud-setups` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="cloud-setups") for rec in records]   # records = your JSON output
